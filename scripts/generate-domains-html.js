...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  h2 { margin:44px 0 4px; font-size:21px; border-left:5px solid; padding-left:10px; }
  h2 small { color:var(--ink-2); font-weight:600; font-size:13.5px; margin-left:8px; }
  h3 { margin:20px 0 8px; font-size:15.5px; }
  h3 small { color:var(--ink-2); font-weight:700; font-size:12px; margin-left:6px; }
...
<div class="wrap">
  ...
  <div class="treemap" role="img" aria-label="4대 도메인별 스킬 분포 트리맵">
    ${blocks
      .map(({ college, subs, skillCount }) => {
        const color = COLLEGE_COLORS[college.id];
        return `<div class="tcol" style="flex-grow:${skillCount}">
      <div class="thead" style="background:${color}"><span>${esc(college.name)}</span><span>${skillCount}</span></div>
      <div class="tstack">
        ${subs
          .map(
            (sub) => `<div class="tile" style="flex-grow:${sub.skills.length};border-left-color:${color};background:color-mix(in srgb, ${color} 14%, #fff)">
          <b>${esc(sub.name)}</b><i>${sub.skills.length}</i></div>`,
          )
          .join("")}
      </div>
    </div>`;
  ...
  <section>
    ...
    ${subs
      .map(
        (sub) => `
    <h3>${esc(sub.name)} <small>${sub.skills.length}개</small></h3>
    <table>
