...
`/evaluation`은 도메인 단위 평가 화면입니다. **4대 도메인 직접 평가가
메인**이고 기능 도메인 평가가 서브이며, 서브 평가의 스킬 수 가중 평균이 메인
카드에 참고치(롤업)로 표시됩니다. 평가는 로그인 신원으로 서버에 아카이빙되고,
도메인 카드마다 하위 스킬 조회와 스킬별 도메인 변경요청도 접수할 수 있습니다.
`/evaluation/skills`는 스킬 단위 평가 전용 화면입니다. 스킬 평가 워크벤치는 사전 지정 평가자 명부(`public/data/evaluators.json`)
