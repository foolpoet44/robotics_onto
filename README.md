...
`/evaluation`은 도메인 단위 평가 화면으로, 도메인 카드마다 하위 스킬을
조회하고 스킬별 도메인 변경요청(기능 도메인/4대 도메인, 로그인 필요)을 접수할
수 있습니다. `/evaluation/skills`는 스킬 단위 평가 전용 화면입니다. 스킬 평가 워크벤치는 사전 지정 평가자 명부(`public/data/evaluators.json`)
